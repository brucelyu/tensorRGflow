...
#
###############################
# funtions for FET starts... (not used in my tensor RG paper)
...
def gilt_halfplaq(A,B,epsilon = 1e-7, convergence_eps = 1e-2,verbose = True):
    """
    Apply gilts on four legs repeatly until done. The plaquette looks like
        | (I) |
     ---A-----B---
    (IV)|     |(II)
        |     |
     ---B-----A---
        |(III)|
    and we truncate the leg according to order I -> III -> II -> IV
    Parameters
    ----------
    A : Four leg tensor A[i,j,k,l]
         j|
...
def applyRpFull(A, Rup, Rvp, Leg = "I", Ruvs = [1]*4, only_ret_A = False):
    """
    Given a plaquette [A, A, A, A] that looks like
        | (I) |
     ---A-----A---
    (IV)|     |(II)
        |     |
     ---A-----A---
        |(III)|
    and also the leg to be cut. Apply Rup and Rvp on A so that
    the leg inside this loop is properly truncated.
    Here we will truncate all the loops in the plaquettes, so it is
    different from Fig.4 of Markus's paper.
    For example, if Leg = "I",
    Amod[i,j,k,l] = sum_n A[m,j,n,l] * Rup[n,k] * Rv[m,i]
    Parameters
    ----------
    A : Four leg tensor A[i,j,k,l]
         j|
    ...
    Leg : str, choose among ("I","II","III","IV")
        the leg to be truncated.
    Ruvs: list, [Rleft, Rup, Rright, Rlower]
        list to keep track of matrices used for leg truncations
    Returns
    -------
    Amod : Four leg tensor
        with one leg properly truncated.
    ...
    """
    Alist = [A, A, A, A]
    [Arot1, _, _, _], curperm = rotateLattice(Alist, Leg)
    Arot1mod = ncon([Arot1, Rup, Rvp],[[1,-2,2,-4],[2,-3], [-1,1]])
    if not only_ret_A:
        Rrotleft, Rrotup, Rrotright, Rrotlower = [Ruvs[k] for k in curperm]
        if type(Rrotright).__module__.split(".")[0] != 'abeliantensors':
            Rrotright = 1.0 * Rup
        else:
            Rrotright = ncon([Rrotright, Rup], [[-1,1],[1,-2]])
        if type(Rrotleft).__module__.split(".")[0] != 'abeliantensors':
            Rrotleft = 1.0 * Rvp.transpose([1,0])
        else:
            Rrotleft = ncon([Rrotleft, Rvp], [[-1,1], [-2,1]])
        Ruvrotlist = [Rrotleft, Rrotup, Rrotright, Rrotlower]
    Amodlist = [Arot1mod, Arot1mod, Arot1mod, Arot1mod]
    revLeg = {"I":"I", "II":"IV", "III":"III", "IV":"II"}
    [Amod, _, _, _], revperm = rotateLattice(Amodlist, revLeg[Leg])
    if only_ret_A:
        return Amod
    Rleft, Rup, Rright, Rlower = [Ruvrotlist[k] for k in revperm]
    return Amod, Rleft, Rup, Rright, Rlower

def gilt_fullplaq(A,epsilon = 1e-7, convergence_eps = 1e-2,verbose = True,
                  return_Ruv = False,
                  loop_red_scheme = "Gilt", argsFET = {}):
    """
    Fully truncate the CDL loops in the plaquettes
   ...
   convergence_eps : float, optional
    ...
    verbose : boolean, optional
        print information. The default is True.
    return_Ruv: boolean, optional
        if Ture, the matrices Ru's and Rv's will also be returned,
        They are useful to calculate the response matrix from which
        the scaling dimensions can be extracted. The defult is False
    Returns
    -------
    A : Four leg tensor A[i,j,k,l]
         j|
    ...
    if return_Ruv is True, also return
    Rleft: truncation matrix acting on left leg of a tensor
    Rright: truncation matrix acting on right leg of a tensor
    Rup: truncation matrix acting on upper leg of a tensor
    Rlower: truncation matrix acting on lower leg of a tensor
    ...
    Rright = 1
    Rlower = 1
    Ruvs = [Rleft, Rup, Rright, Rlower]
    if verbose:
        print("Start Applying {:s}...".format(loop_red_scheme))
    if loop_red_scheme == "Gilt":
        for leg in itt.cycle(ord_leg):
            Ruprime, Rvprime, done, err, nRecur = cutLeg(A, A, leg,
                                               epsilon, convergence_eps, verbose = verbose)
            A, Rleft, Rup, Rright, Rlower = applyRpFull(A,Ruprime, Rvprime, leg, Ruvs)
            Ruvs = [Rleft, Rup, Rright, Rlower]
            # ignore whether it is done or not
            done_legs[leg] = True
            loop_red_err += err
        ...
        for leg in ord_leg:
            up, sp, vp, err, loop_red_err_cur, N_iter = fetCut(A, A, leg, chitid, maxiter, forwhat = "hotrg",
                                initscheme = initscheme, giltdeg = giltdeg, verbose = False)
...
def gilt_iso_fullplaq(A,epsilon = 1e-7, convergence_eps = 1e-2,verbose = True,
                  return_Ruv = False):
    """
    Fully truncate the CDL loops in the plaquettes
   ...
   convergence_eps : float, optional
    ...
    # order of leg name for cutting
    ord_leg_first = ['I','II']
    ord_leg_second = ['III', 'IV']
    ord_leg = [ord_leg_first, ord_leg_second]
    done_legs = {i: False for i in ['I','II','III','IV']}
    # total gilt error, which is simply the sum of all truncation errors
    gilt_err = 0
    # store matrices used for leg truncations
    Rleft = 1
    ...
    for ord_leg_sub in itt.cycle(ord_leg):
        Ruplist = []
        Rvplist = []
        for leg in ord_leg_sub:
            Ruprime, Rvprime, done, err, nRecur = cutLeg(A, A, leg,
                                               epsilon, convergence_eps, verbose = False)
            Rleft, Rup, Rright, Rlower = applyRpFull(A,Ruprime, Rvprime, leg, Ruvs)[1:]
            Ruvs = [Rleft, Rup, Rright, Rlower]
            Ruplist.append(Ruprime)
            Rvplist.append(Rvprime)
            # whether the leg is done or not
            done_legs[leg] = done
            gilt_err += err
        for k in range(len(Ruplist)):
            Ruprime = Ruplist[k]
            Rvprime = Rvplist[k]
            A =  applyRpFull(A, Ruprime, Rvprime, ord_leg_sub[k],
                             only_ret_A = True)
...
if __name__ == "__main__":
    ...
    import argparse
    from Isings import exactSol
    from trg import Ising2dT, Levin07TRG

    def mainTRG(relT,chi,iterN = 12, isGilt = False,
                isSym = False, verbose = False, gilteps = 1e-5):
        Tc = 2 / np.log(1 + np.sqrt(2))
        beta = 1 / Tc / relT
        A0 = Ising2dT(beta, isSym = isSym)
        Anorm = [0 for x in range(iterN +1)]
        Anorm[0] = A0.norm()
        A = [0 for x in range(iterN +1)]; A[0] = A0 / Anorm[0]
        # enter RG loop
        for k in range(iterN):
            print("{:d}-th iteration of TRG!".format(k))
            if isGilt:
                Aten, Bten = gilt_halfplaq(A[k], A[k], verbose = verbose,
                                           epsilon = gilteps,
                                           convergence_eps = 1e-1)
            else:
                Aten = A[k].copy()
                Bten = A[k].copy()
            Aprime = Levin07TRG(Bten,Aten,chi, verbose)
            Aprime = Levin07TRG(Aprime,Aprime, chi, verbose)
            Aprime = Aprime.transpose([3,0,1,2])
            Anorm[k+1] = Aprime.norm()
            A[k+1] = Aprime / Anorm[k+1]
            if verbose:
                _,s,_ = A[k+1].svd([0,1],[2,3])
                s = s/s.max()
                s = s.to_ndarray()
                s = np.abs(s)
                s = -np.sort(-s)
    ...
    def calg(A,Anorm,initN = 4, traceord = [1,1,2,2]):
        ...
        for k in range(1,numlevels+1):
            FreeEnergy[k-1] = (np.sum(4.0 ** (-np.arange(0,k+1)) *
                                      np.log(Anorm[:(k+1)])) +
                               (1/4)**k * np.log(
                                   ncon([A[k]],[traceord]).norm())) /(2*initN)
        return FreeEnergy
    ...
    if func == "envLeg":
    ...
    elif func == "gilt_halfplaq":
        print("Test client of function gilt_halfplaq")
        A,B = gilt_halfplaq(cdlT,cdlT)
        ucut,s,vcut = B.svd([0,1],[2,3])
        print("Spectrum of tensor B is")
        print(s.to_ndarray())
    elif func == "gilt_fullplaq":
        print("Test client of function gilt_fullplaq")
        A, Ruvs = gilt_fullplaq(cdlT, return_Ruv = True,
                                loop_red_scheme = loopred,
                                    argsFET = argsFET)
        print("CDL tensor after truncation is")
        print(A.to_ndarray())
        print("Check whether we successfully keep track of Ruv matrices...")
        print("Calculating truncated A using Ruv matrices...")
        Rl, Ru, Rr, Rlo = Ruvs
        Acheck = ncon([cdlT,Rl, Ru, Rr, Rlo],[[1,2,3,4],[1,-1],[2,-2],[3,-3],[4,-4]])
        print("CDL tensor after surrounded by Ruvs matrics is")
        print(Acheck.to_ndarray())
        print("Is two tensor equal? ")
        print(A.allclose(Acheck))
        print("Error of exact solution is ")
        print(np.abs(A.norm() - numb)/numb)
    elif func == "gilt_iso_fullplaq":
        print("Test client of function gilt_iso_fullplaq")
        A, Ruvs = gilt_iso_fullplaq(cdlT, return_Ruv = True)
        print("CDL tensor after truncation is")
        print(A.to_ndarray())
        print("Check whether we successfully keep track of Ruv matrices...")
        print("Calculating truncated A using Ruv matrices...")
        ...
        T = Tc * relT
        gext = exactSol([1/T])[0]
        print("Relative temperature = {:.4f}".format(relT))
        print("Bound dimension chi = {:d}".format(chi))
        print("Ordinary TRG...")
        A, Anorm = mainTRG(relT, chi, 24)
        print("Error of free energy")
        approxg = calg(A,Anorm, initN = 1,  traceord = [1,2,1,2])
        err = np.abs(approxg - gext) / gext
        print(err)
        print("--------------------------------------")
        print("Gilt-TRG...")
        AGilt, AnormGilt =  mainTRG(relT, chi, 24, True, True, True,
                                    gilteps = gilteps)
        print("Error of free energy (add gilts)")
        approxg = calg(AGilt,AnormGilt, initN = 1,  traceord = [1,2,1,2])
        err = np.abs(approxg - gext) / gext
        print(err)
