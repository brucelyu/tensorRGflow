...
\end{align}
%
To see why the gauge fixing procedure in Eqs.~\eqref{def:Nx} to~\eqref{def:gaugefixHori} defines a preferred set of bases, let us examine how the tensor $\tilde{A}$ in Eq.~\eqref{def:gaugeTrans} transforms.
...
The arguments can be trivially generalized to complex tensors.
%
...
\end{align}
%
where $S_x,S_y$ are orthogonal matrices (the justification for the restriction to orthogonal matrices will come later).
...
We still have one last question to answer: why can we restrict invertible matrices $S_x, S_y$ in the gauge transformation to be orthogonal matrices? 
This restriction is justified if the initial tensor $A$ of the system has spatial reflection symmetries, $A_{ijkl} = A_{kjil} = A_{ilkj}$ (see Eq.~\eqref{def:gaugeTrans} for the legs' order convention).
This means that the transfer matrix $N_x$ in Eq.~\eqref{def:Nx} obtained by contracting two vertical legs of $A$ is symmetric.
The transfer matrix constructed from the coarser tensor $A_c$ remains symmetric during the exact block-tensor transformation in Eq.~\eqref{eq:exactBlock}. 
The approximation procedures in the GILT and the HOTRG preserve the symmetry of transfer matrices $N_x, N_y$.
It can be seen by examining the symmetry properties of pieces of low-rank $Q_A, Q_B$ matrices and of the isometry $w$ in Eq.~\eqref{def:ApycontrGilt}.
The low-rank matrices $Q_A, Q_B$ in Fig.~\ref{fig:gilt4hotrg} inherit the reflection symmetry of the tensor $A$, so they are both symmetric matrices.
Furthermore, $Q_A$ and $Q_B$ are related to each other by a horizontal reflection since their environment plaquettes (see Eq.~\eqref{eq:bondEnvSVD}) are related to each other through such a symmetry transformation.
These two symmetry properties of $Q_A$ and $Q_B$ ensures that the two transfer matrices $N_x, N_y$ constructed from the patch of tensors in the dashed circle in Fig.~\ref{fig:gilt4hotrg} are symmetric matrices.
The subsequent HOTRG acts the isometry $w$ and its transpose on this patch of tensors, which also preserves the reflection symmetry of two transfer matrices.
Consequently, the approximation procedures in the GILT-HOTRG preserve the symmetry of transfer matrices.
This symmetry-preserving property of the GILT-HOTRG enforces the invertible matrices $S_x, S_y$ to be orthogonal matrices, because a general invertible matrix will break the reflection symmetry of transfer matrices. 
%

This restriction to orthogonal matrices is further supported by the numerical evidence in Sec.~\ref{benchmark:2DIsing}.
