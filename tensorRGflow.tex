...
%this restricts $\chi \leq 8$ for the fixed-point analysis in tensor space.
...
\begin{figure}[tb]
    ...
    \scalebox{0.82}{\tikzfig{summaryintro}}
...
After the spin variables shared by four $A$ tensors forming the same $A_c$ are summed over, we are left with four bigger squares, with two spin variables sitting on each side of each square.
...
In comparison, in the conventional Hamiltonian approach, we fix the form of the new Hamiltonian $\mathcal{H}'$ to be the same as the old one $\mathcal{H}$ in Eq.~\eqref{eq:generalspinHam}, with $\mathcal{H}' $ characterized by a set of new coupling constants $\mathbf{K}'$. 
